...
// server su /api/login (così /users resta protetto da regole Firebase
// chiuse senza rompere il login — vedi api/login.js).
//
// Le candidature vengono lette da /api/applications (non più da Firebase
// direttamente dal browser): il server richiede un token di sessione
// valido, rilasciato al login, così nessuno può leggere i dati dei
// candidati senza autenticarsi. Vedi README.md.
// ---------------------------------------------------------------
...
loginForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (data.ok && data.token) {
      ...
      errMsg.textContent = "Il server non ha SESSION_SECRET configurato (vedi README).";
    } else {
